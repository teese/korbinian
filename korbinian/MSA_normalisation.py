...
def cal_aa_propensity (df):
    # input: dataframe which contains the TM and non-TM sequences

    # output: pandas dataframe which show the aa propensity in TM and non-TM region, respectively

    # create a string to hold TM segments from all proteins as merged together
    massive_string_tm = ""
    for seq in df['TM01_seq']:
        if type(seq) == str:
            massive_string_tm += seq

    # create a string to hold non-TM segments from all proteins as merged together
    massive_string_ntm = ""
...
def count_aa_freq(seq):
    ...
    # input: string (TM or non-TM sequence)
    # output: pd.DataFrame

    # count absolute number of each aa in the input string
    aa_dict = {}
    for aa in seq:
        aa_dict[aa] = 0
    for aa in seq:
    ...
    # create a dictionary to hold the propensity of each aa
    prop_dict = {}
    for aa in aa_dict:
        prop_dict[aa] = aa_dict['%s' % aa] / len(seq)

    # turn the dictionary into a pd.DataFrame and transpose it so that the aa names are listed in index
...
def cal_random_aa_ident(prob_table, ident=0.9, seq_len=10000, number_seq=500, subset_num=12):
    ...
    # input:
    # prob_table: pd.DataFrame with aa propensity for TM and non-TM region. obtained from the function cal_aa_propensity
    # ident: overall identity of randomly created sequence cluster. Default value: 90%
    # seq_len: length of randomly created sequences. To achieve a more plausible result using randomisation method,
        # greater values (> 5000) are recommended. Defalut value: 10,000
    # number_seq: number of aligned sequences. Larger values are recommended. Default value: 500
    # subset_num: currently not in use.

    # output: Tuple in the form of (random identity TM, random identity non-TM)
    ...
    cons_df2.index = list(nonTM_orig_seq)

    aa_prop_list = []
    for aa in cons_df2.index.unique():
        if type(cons_df2[aa][aa]) == pd.Series:
            observed_cons_1_row = list(cons_df2.loc[aa, aa])
            aa_prop_list += observed_cons_1_row
        else:
            aa_prop_list.append(cons_df2.loc[aa, aa])
...
def generate_ran_seq(seq_len, number_seq, number_mutations, subset_num, aa_pool, aa_probabilities):
    ...
    # input:
    # seq_len: length of randomly created sequences. To achieve a more plausible result using randomisation method,
    # greater values (> 5000) are recommended.
    # number_seq: number of aligned sequences. Larger values are recommended.
    # number_mutations: number of residues that will be randomly replaced in a sequence.
    # subset_num: currently not in use.
    # aa_pool: np.array of amino acid from which residues will be chosen randomly to create a sequence or to replace a residue
    # aa_probabilities: np.array of propensities. The order should match the amino acid in aa_ppol

    # output:
    # ori_seq: one original sequence as reference
    # seq_matrix: list of seqeunces that are created by randomly replacing predetermined number of residues in
...
def cal_MSA_ident_n_factor(ident_tm, rand_tm, rand_ntm):
    ...
    # e.g. observed average ident_tm = 0,78, rand_tm = 0.126, rand_ntm = 0.059
    # calculated real_cons = 0.748
    # calculated ident_ntm = 0.763
    # calculated n_factor = 0.78/0.763 = 1.022

    # input:
    # ident_tm: the observed average identity of TM region in your MSA which needs to be normalised
    # rand_tm: random identity in TM region, calculated based on your dataset using radomisation method
    # rand_ntm: random identity in non-TM region, calculated based on your dataset using radomisation method

    # output:
    # n_factor: normalisation factor which will be applied to your observed TM identity
    # TM_ident_n: normalised TM identity for your MSA
