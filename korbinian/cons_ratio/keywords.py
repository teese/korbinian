...
def keyword_analysis(pathdict, s, logging):
    ...
    # remove proteins containing nan in AAIMON_mean_all_TMDs
    list_of_acc_without_nan = []
    for acc in df.index:
        if not pd.isnull(df.loc[acc, 'AAIMON_mean_all_TMDs']):
    ...
    if list_KW_counts_major:
        ...
        for keyword in list_KW_counts_major:
            ...
            dfk.loc[keyword, 'odds_ratio_AAIMON_no_exclusion'] = dfk.loc[keyword, 'AAIMON_keyword_mean'] / dfk.loc[keyword, 'AAIMON_no_keyword_mean']
            ...
            dfk.loc[keyword, 't-value_AAIMON_no_exclusion'] = t
            dfk.loc[keyword, 'p-value_AAIMON_no_exclusion'] = p

            # calculate odds ratio, p- and t-values for AAIMON_slopes
            dfk.loc[keyword, 'odds_ratio_AAIMON_slope_no_exclusion'] = dfk.loc[keyword, 'AAIMON_slope_keyword_mean'] / dfk.loc[keyword, 'AAIMON_slope_no_keyword_mean']
            data1 = df_keyword['AAIMON_slope_mean_all_TMDs']
            data2 = df_no_keyword['AAIMON_slope_mean_all_TMDs']
            t, p = ttest_ind(data1, data2, equal_var=True)             # equal_var True or False ?!?!
            dfk.loc[keyword, 't-value_AAIMON_slope_no_exclusion'] = t
            dfk.loc[keyword, 'p-value_AAIMON_slope_no_exclusion'] = p
            ...
            sys.stdout.write('\nmean AAIMON containing keyword  "{a}" : {b:.3f} ± {c:.3f}, n = {d:.0f}\n'
                             ...
                             .format(a=keyword,
                                     ...
                                     h=dfk.loc[keyword, 'odds_ratio_AAIMON_no_exclusion'],
                                     i=dfk.loc[keyword, 't-value_AAIMON_no_exclusion'],
                                     j=dfk.loc[keyword, 'p-value_AAIMON_no_exclusion'],
                                     ...
                                     o=dfk.loc[keyword, 'odds_ratio_AAIMON_slope_no_exclusion'],
                                     p=dfk.loc[keyword, 't-value_AAIMON_slope_no_exclusion'],
                                     q=dfk.loc[keyword, 't-value_AAIMON_slope_no_exclusion']))


            if dfk.loc[keyword, 'p-value_AAIMON_no_exclusion'] <= s['p_value_cutoff_for_histograms']:
                ###############################################################
                #                                                             #
                ...
                data_keyword = df_keyword['AAIMON_mean_all_TMDs'].values
                ...
                data_no_keyword = df_no_keyword['AAIMON_mean_all_TMDs'].values
                # create numpy array of membranous over nonmembranous conservation ratios (identity)
        ...
        for KW in list_KW_correlation:
            ...
            correlated_keywords = correlated_keywords.drop(KW).sort_values(ascending=False).to_string(max_rows=10)
            dfk.loc[KW, 'correlated_KW'] = correlated_keywords


        # save pandas dataframes with values
        dfk.to_csv(os.path.join(pathdict["keywords"], 'keywords.csv'), sep=",", quoting=csv.QUOTE_NONNUMERIC)   # - transpose dataframe here ?!
        df_correlation.to_csv(os.path.join(pathdict["keywords"], 'cross_KW_correlation.csv'), sep=",", quoting=csv.QUOTE_NONNUMERIC)
