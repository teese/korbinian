...
def OLD_calculate_TMD_conservation(pathdict, set_, logging):
    ...
    df['kept_after_redundancy_check'] = \
        df['kept_after_redundancy_check'].astype(np.bool)
    df['protein_kept_for_statistical_analysis'] = \
        df['protein_kept_for_statistical_analysis'].astype(np.bool)
    df['query_md5'] = df['query_md5'].astype('<U32')
    df['md5_checksums_in_homologue_list_that_are_also_in_query_list'] = \
        df[
        ...
        if csv_file_with_hit_details_exists and csv_file_with_hit_details_contains_hits:
            ...
            if csv_file_with_homologues_is_old:
                logging.warning("%s csv file is old!! Need to recreate csv from XML file of homologues (%s,%s)" % (
                    protein_name[:6], df.loc[i, 'SIMAP_homologues_XML_file_path'],
                    df.loc[i, 'SIMAP_csv_analysed_path']))
            ...
            #                parse_single_homologue_XML_to_csv_and_fasta(df.loc[i,'SIMAP_homologues_XML_file_path'],df.loc[i,'SIMAP_csv_analysed_path'], nested_dict_with_uniprot_seq[row_in_csv_with_protein])
            ...
            df_SIMAP_homologues_kept_for_statistical_analysis_stat_only = df_SIMAP_hits[
                df_SIMAP_hits.match_TMD_kept_for_statistical_analysis == True]
            ...
            logging.info('%s\tnumber_of_valid_hits: %i, protein_kept_for_statistical_analysis: %s' % (
                ...
                df.loc[i, 'protein_kept_for_statistical_analysis']))
            '''Previously, a histogram was created for each protein
            '''
            if df.loc[i, 'protein_kept_for_statistical_analysis']:
    ...
    for ii2 in df.index:
        list_of_query_md5_checksums.append(df.loc[ii2, 'query_md5'])
    ...
    #    array_with_query_md5_data = np.ones(len(df),
    ...
    for i in df.index:
        csv_file_with_hit_details = df.loc[
        ...
        if os.path.isfile(csv_file_with_hit_details):
            ...
            if len(df_SIMAP_hits) != 0:
                ...
                #            df.loc[i,'number'] = i
                #            df.loc[i,'query_name'] = protein_name
                #            df.loc[i,'query_md5'] = query_md5
    ...
    for i in df.index:
        ...
        if df.loc[
            ...
            # array_with_redundant_md5_data = np.ones(len(array_of_redundant_md5s)*4, dtype = [('redundant_md5', '<U33'), ('row_in_df', '<i4'), ('number_of_valid_hits', '<i4'), ('kept_after_redundancy_check', 'b')]).reshape(len(array_of_redundant_md5s),4)
            ...
            if df.loc[i, 'kept_after_redundancy_check'] == True:
                ...
                for h in df_redundant_md5.index:
                    ...
                    row_in_df = int(
                        df[
                            df['query_md5'] == redundant_md5].index)
                    df_redundant_md5.loc[
                        h, 'row_in_df'] = row_in_df
                ...
                # now transfer this data to the original array (df)
                for m in df_redundant_md5.index:
                    row = df_redundant_md5.loc[m, 'row_in_df']
    ...
    # if df.loc[i,'kept_after_redundancy_check'] != 'False':
    #
    ...
    #        row_in_df = list_of_query_md5_checksums.index(array_with_redundant_md5_data[i]['redundant_md5'])
    #        array_with_redundant_md5_data[i]['row_in_df'] = row_in_df
    #        #use this row_in_df to find the number of valid hits
    #        array_with_redundant_md5_data[i]['number_of_valid_hits'] = array_with_query_md5_data[row_in_df]['number_of_valid_hits']
    #        array_with_redundant_md5_data[i]['query_name'] = array_with_query_md5_data[row_in_df]['query_name']
    #
    #
    ...
    #        row_in_df = array_with_redundant_md5_data[i]['row_in_df']
    #        array_with_query_md5_data[row_in_df]['kept_after_redundancy_check'] = array_with_redundant_md5_data[i]['kept_after_redundancy_check']
    # logging.info(array_with_redundant_md5_data)
    #
    # find row in dataframe that contains the redundant md5
    #    index_row_in_df_containing_redundant_md5 = int(df[df['query_md5']==redundant_md5].index)
    #    #now update the df to show that this protein should be regarded as redundant
    # df.loc[index_row_in_df_containing_redundant_md5,'kept_after_redundancy_check'] = array_with_redundant_md5_data[i]['kept_after_redundancy_check']
    ...
    try:
        df_dfout07_simapnonred = \
            df[
                df.kept_after_redundancy_check == True]
    except KeyError:
